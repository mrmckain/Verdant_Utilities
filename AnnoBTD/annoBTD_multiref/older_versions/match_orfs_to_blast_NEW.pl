...
while(<$file>){
		...
		my @tarray = split /\s+/;
		for my $seq (@{$blastgenes{$tarray[1]}}){
		my $hit_truelen=length($seq);
		...
		if($hit_truelen > 4500){
			...
			if($hit_curlen/$hit_truelen >= 0.35){
				$idorfs_f{$tarray[1]}{$tarray[0]}{"Start"}=$orf_pos{$tarray[0]}{"Start"};
				$idorfs_f{$tarray[1]}{$tarray[0]}{"End"}=$orf_pos{$tarray[0]}{"End"};
				if(exists $blast_overlaps{$tarray[1]}){
					if(exists $blast_overlaps{$tarray[1]}{$tarray[0]}){
...
for my $shid (keys %short_exons){
		...
		if($shid =~ /trn/){
			...
			for my $seq (@{$blastgenes{$shid}}){
				if($tseq){
				if(length($tseq) > length($seq)){
					next;
				}
				else{
					$tseq=$seq;
...
while(<$tfile>){
		...
		if(/>/){
				...
				$temphash{$sid}[$seq_counter{$sid}].=$_;
...
sub exon_mods_start {
	...
	my $temp_orf_len = $idorfs_f{$gid}{$eid}{"End"}-$idorfs_f{$gid}{$eid}{"Start"};
	my $orf_seq_keep;
	my $diff_seq_keep;
	for my $temp_orf_seq (@{$blastgenes{$gid}}){
		if($orf_seq_keep){
			if(abs(length($temp_orf_seq)-$temp_orf_len)< $diff_seq_keep){
				$orf_seq_keep = $temp_orf_seq;
				$diff_seq_keep = abs(length($temp_orf_seq)-$temp_orf_len);
			}
		}
		else{
			$orf_seq_keep = $temp_orf_seq;
	...
	my $gene_seq=substr($orf_seq_keep,0, length($orf_seq_keep)-$frame_restore);
...
sub exon_mods_end {
	...
	my $gene_seq=substr($orf_seq_keep,$frame_restore, length($orf_seq_keep));
	my $orf_seq = substr($plastome,$idorfs_f{$gid}{$eid}{"Start"},($idorfs_f{$gid}{$eid}{"End"}-$idorfs_f{$gid}{$eid}{"Start"}+1));
	my $rev_orf_seq = reverse($orf_seq);
	$rev_orf_seq =~ tr/ATCGatcg/TAGCtagc/;
	my $orf_seq_1=substr($orf_seq,1);
	...
	if($formatch>$revmatch){
		...
		if(substr($gene_prot, -1) eq "_"){
			$end_match = index($rev_orf_prot,"_",$nearstart);
			$end_match = length($rev_orf_seq)-($end_match*3 + 2)-1;
			#$end_match = length($rev_orf_seq) - $end_match;
		}
		else{
...
sub exon_mods_mid{
	...
	$gene_seq = substr($orf_seq_keep, $frame,length(substr($gene_seq,$frame))-$end_remove);
	my $orf_seq = substr($plastome,$idorfs_f{$gid}{$eid}{"Start"},($idorfs_f{$gid}{$eid}{"End"}-$idorfs_f{$gid}{$eid}{"Start"}+1));
	my $rev_orf_seq = reverse($orf_seq);
	$rev_orf_seq =~ tr/ATCGatcg/TAGCtagc/;
	my $orf_seq_1=substr($orf_seq,1);
