...
while(<$file>){
		...
		$tarray[1] =~ /(.*?)XXX.+/;
		my $temp_geneid = $1;
		$best_orf_scores_id{$tarray[1]}{$tarray[0]}=$tarray[2];
...
for my $bid (sort keys %idorfs){
	...
	if(scalar keys %{$idorfs{$bid}}>1){
		my %temporfs;
		#In this instance, $bid = gene name and $oid = orf name
		# $toid is the temporary orf name
				...
				if(%temporfs){
						...
						for my $toid (keys %temporfs){
							if($temporfs{$toid}{"Start"} > $cur_start && $temporfs{$toid}{"End"} < $cur_end){
								if($best_orf_scores_id{$bid}{$toid} > $best_orf_scores_id{$bid}{$oid}){
...
while(<$tfile>){
		...
		if($hit_curlen/$hit_truelen >= 0.96 && $tarray[2] >= 85){
...
sub alt_start{
	...
	for (my $i=0; $i<($_[1]*3+1);$i++){
		if(index($seq,substr($_[2],0,6),$i)>=0){#third parameter is gene_seq
			if(index($seq,substr($_[2],0,6),$i) < ($_[1]*3+1)){
				$matchstart =index($seq,substr($_[2],0,6),$i);
				$i=$matchstart;
			}
		}
	}
...
sub match_end{
	...
	####previously $tempend = length($seq)*3-($tempend*3 + 2)-1;
